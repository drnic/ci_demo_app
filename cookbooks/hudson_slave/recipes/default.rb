...
if ['solo','app_master'].include?(node[:instance_role]) && env_name =~ /_(ci|hudson_slave)$/
  ...
  execute "install_hudson_in_resin" do
    command "/usr/local/ey_resin/ruby/bin/gem install hudson -v 0.3.0.beta.3"
    not_if { FileTest.directory?("/usr/local/ey_resin/ruby/gems/1.8/gems/hudson-0.3.0.beta.3") }
  end
